...
in {
  ...
  external-secrets.helm = ["https://charts.external-secrets.io" "0.18.0"];
  flux-operator.helm = ["oci://ghcr.io/controlplaneio-fluxcd/charts" "0.23.0"];
  flux.github-releases = ["fluxcd/flux2" "2.6.3"];
  ...
  homepage.docker = ["ghcr.io/gethomepage/homepage" "1.3.2" vp];
  ...
  ingress-nginx.helm = ["https://kubernetes.github.io/ingress-nginx" "4.12.3"];
  jaeger-collector.docker = ["jaegertracing/jaeger-collector" "1.70.0"];
  jaeger-query.docker = ["jaegertracing/jaeger-query" "1.70.0"];
  jaeger.helm = ["https://jaegertracing.github.io/helm-charts" "3.4.1"];
  k0s.docker = ["attilaolah/k0s" "1.33.2.0"];
  k0s.github-releases = ["k0sproject/k0s" "1.33.2+k0s.0" vp];
  keycloak.helm = ["oci://registry-1.docker.io/bitnamicharts" "24.7.5"];
  kube-prometheus-stack.helm = ["https://prometheus-community.github.io/helm-charts" "75.7.0"];
  kubelet-csr-approver.helm = ["https://postfinance.github.io/kubelet-csr-approver" "1.2.10"];
  kubernetes.github-releases = ["kubernetes/kubernetes" "1.33.2" vp];
  local-path-provisioner.github-releases = ["rancher/local-path-provisioner" "0.0.31" vp];
  loki.helm = ["https://grafana.github.io/helm-charts" "6.30.1"];
  ...
  oauth2-proxy.docker = ["quay.io/oauth2-proxy/oauth2-proxy" "7.9.0" vp];
  pause.docker = ["registry.k8s.io/pause" "3.9"];
  reloader.helm = ["oci://ghcr.io/stakater/charts" "2.1.4"];
